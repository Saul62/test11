    ...
    if predict_button:
        ...
        try:
            ...
            try:
                ...
                st.subheader("特征贡献分析（本例 |SHAP|）")
                values = np.abs(np.array(shap_value))

                order = np.argsort(values)[::-1]
                names_sorted = [feature_dict.get(current_features[i], current_features[i]) for i in order]
                vals_sorted = values[order]

                fig_gc, ax_gc = plt.subplots(figsize=(8, 6))
                bars = ax_gc.barh(range(len(order)), vals_sorted, color='#ff375f')
                ax_gc.set_yticks(range(len(order)))
                ax_gc.set_yticklabels(names_sorted)
                ax_gc.invert_yaxis()
                ax_gc.set_xlabel('|SHAP|（本例）')
                ax_gc.set_title('特征贡献分析（本例 |SHAP|）')
                # 标注 +0.xx
                for i, b in enumerate(bars):
                    w = b.get_width()
                    ax_gc.text(w + max(vals_sorted)*0.02, b.get_y() + b.get_height()/2,
                               f"+{w:.02f}", va='center', fontsize=10, color='#ff375f')
                plt.tight_layout()
                st.pyplot(fig_gc)
                plt.close(fig_gc)
